 ...
 * @author SMART
 */
public class Update_Fees_Due_Record extends JFrame {

    int ID;

    public Update_Fees_Due_Record(JTable table, int txt) {
        this.ID = txt;
        JPanel p1 = new JPanel();
        setTitle("Update Due Fee Record");
        setSize(900, 700);
        setLocation(250, 20);
        setVisible(true);
        ...
        JLabel lb = new JLabel("Update Due Fee Record Page");
...
// when button is pressed on The X icon system will exit//
    ...
    public void updateDueFeeRecord(String txt2, String txt3, String txt4, String txt5, int txt6, JTable table) throws ClassNotFoundException, SQLException {
        ...
        stmt.executeUpdate("UPDATE fees_due "
                + " SET tuitionfee_due = "
                + "" + txt2 + " ," + " pocketmoney_due = " + txt3 + "," + " cautionmoney_due = " + txt4 + "," + " student_id = " + txt5 + " Where feesdue_id = " + txt6 + ";");
        Display_Due_Fees disp = new Display_Due_Fees();
        connection.close();
    }
}
